                     ...
                     const long long   repeat)
{
  ...
  set_vector<<<n_blocks, block_size>>>(N, 1.f, v2);
  ...
  for (unsigned int t = 0; t < n_tests; ++t)
    {
      ...
      matmul<<<n_blocks, block_size>>>(v1, v2, v3, N, N);
  ...
  cudaMemcpy(result_host.data(), v3, N * N * sizeof(float), cudaMemcpyDeviceToHost);
